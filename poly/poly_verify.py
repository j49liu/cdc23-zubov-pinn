...
def CheckLyapunov_c(x, f, V, c1, c2, config, epsilon):    
    ...
    x_boundary = logical_or(x[i] == xlim[1], x_boundary)
    x_boundary = logical_or(x[i] == -xlim[1], x_boundary)

    set_inclusion = logical_imply(x_bound, logical_not(x_boundary))  
    stability = logical_imply(logical_and(x12_bound, V <= c1), 
...
result= CheckLyapunov_c(vars_, f_ex, V_learn, c1, c2, config, epsilon) 
...
if (result): 
  print(f"Not a Lyapunov function on V<={c2}. Found counterexample: ")
  print(result)
else:
  print("Satisfy conditions with beta = ", epsilon)
  print(V_learn, f" is a Lyapunov function on V<={c2}.")
